...
async function awaitTransactionSignatureConfirmation(
  ...
  status = await new Promise(async (resolve, reject) => {
    ...
    try {
      log.info('awaitTransactionSignatureConfirmation :: txid', txid);
      subId = connection.onSignature(
        txid,
        (result, context) => {
          log.info('awaitTransactionSignatureConfirmation :: result', result);
          log.info('awaitTransactionSignatureConfirmation :: context', context);
          done = true;
          status = {
            err: result.err,
            slot: context.slot,
