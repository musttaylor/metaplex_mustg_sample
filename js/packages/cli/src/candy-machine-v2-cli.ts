...
programCommand('mint_one_token')
  ...
  .action(async (directory, cmd) => {
	...
	log.info('mint_one_token started');
	//log.info('mint_one_token keypair', keypair);
	//log.info('mint_one_token env', env);
	//log.info('mint_one_token cacheName', cacheName);
	//log.info('mint_one_token rpcUrl', rpcUrl);
	
	
    const cacheContent = loadCache(cacheName, env);
	log.info('mint_one_token cacheContent', cacheContent);
